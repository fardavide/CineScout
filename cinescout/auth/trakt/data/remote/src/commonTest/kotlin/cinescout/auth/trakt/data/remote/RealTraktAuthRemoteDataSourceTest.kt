...
class RealTraktAuthRemoteDataSourceTest {
    ...
    private val authService: TraktAuthService = mockk {
        ...
        coEvery { createAccessToken(refreshToken = TraktRefreshToken(any())) } returns
            RemoteTraktAuthTestData.CreateAccessTokenResponse.right()
    }
    ...
    fun `create access token from code`() = runTest {
        // given
        val expected = TraktAccessAndRefreshTokensSample.AccessAndRefreshToken.right()
    ...
    fun `create access token from refresh token`() = runTest {
        ...
        // when
        val result = dataSource.createAccessToken(TraktRefreshTokenSample.RefreshToken)

        // then
        coVerify { authService.createAccessToken(TraktRefreshTokenSample.RefreshToken) }
        assertEquals(expected, result)
    }
}
