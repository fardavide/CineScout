...
import cinescout.lists.presentation.previewdata.RatedListScreenPreviewDataProvider
...
@Composable
fun RatedListScreen(state: RatedListState, modifier: Modifier = Modifier) {
    Box(
        modifier = modifier
            .fillMaxSize()
        ...
        when (state) {
            is RatedListState.Error -> ErrorScreen(text = state.message)
            RatedListState.Loading -> CenteredProgress()
            is RatedListState.Data -> ListContent(data = state)
        }
    }
}

@Composable
private fun ListContent(data: RatedListState.Data) {
    when (data) {
        RatedListState.Data.Empty -> EmptyListContent()
        is RatedListState.Data.NotEmpty -> NotEmptyListContent(items = data.items)
    }
}
...
private fun EmptyListContent() {
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Text(
            text = stringResource(id = string.lists_rated_empty),
...
fun RatedListScreenPreview(
    @PreviewParameter(RatedListScreenPreviewDataProvider::class) state: RatedListState
) {
    CineScoutTheme {
